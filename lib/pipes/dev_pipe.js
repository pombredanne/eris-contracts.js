...
DevPipe.prototype.transact = function (txPayload, callback) {
    ...
    if(txPayload.from){
        var fromAcc = this._accountData.account[txPayload.from];
        if(!fromAcc){
...
DevPipe.prototype.call = function (txPayload, callback) {
    ...
    if(!from) {
        from = this._accountData.default.address;
    }
...
function _createAccountData(accounts){
    ...
    if(accounts && accounts.length > 0){
        accountData.default = accounts = accounts[0].address;
        for(var i = 0; i < accounts.length; i++){
            accountData.account[accounts[i].address] = accounts[i];
        }
    }
    return accountData;
}
